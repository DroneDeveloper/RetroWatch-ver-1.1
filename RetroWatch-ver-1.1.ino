...
/*
Retro Watch Arduino v1.1

version UPDATE(2015.09.04)
1.i just added to a blutooth icon.
2. and a battery state icon.
3. and Light function.
4. and measuring of temperature.
5. and a vibrating motor.
6. and some buttons.

Written by Hyen Jun Kim (foo89@naver.com)
if you have any question, please contact me this site. https://www.facebook.com/hyenjun.kim
*/
...
int buttonPin2 = 5;
int buttonPin3 = 6;
int BlueToothPin = A3; // bluetooth LED pin
int motorPin = 10;
boolean isClicked = false;
boolean delayDisplay = false;
unsigned long time1;
unsigned long time2;
//////////////////////MENU////////////////////////////
int menuState = 0;
...
void setup()   {
  ...
  pinMode(buttonPin, INPUT);     digitalWrite(buttonPin, HIGH);// Defines button pin
  pinMode(buttonPin2, INPUT);    digitalWrite(buttonPin2, HIGH);// Defines button pin
  pinMode(buttonPin3, INPUT);   digitalWrite(buttonPin3, HIGH);  // Defines button pin
...
void loop() {
  ...
  delayDisplay = isDisplayTime(current_time);
  bettenfunction();
...
void onDraw(unsigned long currentTime) {
  if(!delayDisplay)    // Do not re-draw at every tick
    return;
...
void drawIndicator() {
  if(updateIndicator) {
    ...
    if(msgCount > 0) {
      display.drawBitmap(127 - 25, 5, IMG_indicator_msg, 8, 8, WHITE);
      display.setTextColor(WHITE);
      display.setTextSize(1);
      display.setCursor(127 - 32, 5);
      display.print(msgCount);
      drawCount++;
    }
    
    if(emgCount > 0) {
      display.drawBitmap(127 - 25*drawCount - 7*(drawCount-1), 5, IMG_indicator_emg, 8, 8, WHITE);
      display.setTextColor(WHITE);
      display.setTextSize(1);
      display.setCursor(127 - 25*drawCount - 7*drawCount, 5);
...
void drawClock() {
  ...
  // CLOCK_STYLE_SIMPLE_MIX
    ...
    display.drawCircle(centerY, centerY+6, iRadius - 6, WHITE);
    showTimePin(centerY, centerY+6, 0.1, 0.4, iHour*5 + (int)(iMinutes*5/60));
    showTimePin(centerY, centerY+6, 0.1, 0.70, iMinutes);
...
double getBandgap(void) 
    {
     ...
     double results = (((InternalReferenceVoltage * 1024L) / ADC) + 5L) / 10L; 
     return results;
...
void BTConnect()
{
  if(analogRead(BlueToothPin)>=600)
  {
  drawIcon(1,0,64);//ICON_BITMAP_wifi
  display.display();
  }
  else
  { 
    display.drawLine(6,8, 14, 8, BLACK);
    display.drawLine(6,9, 14, 9, BLACK);
    display.drawLine(6,10, 14, 10, BLACK);
    display.drawLine(6,11, 14, 11, BLACK);
    display.drawLine(6,12, 14, 12, BLACK);
    display.drawLine(6,13, 14, 13, BLACK);
...
void BatteryIndicator()
 {
    if((battVolts>340)&&(battVolts<=349))drawIcon(110,1,7);
    if((battVolts>349)&&(battVolts<=360))drawIcon(110,1,6);
    if((battVolts>360)&&(battVolts<=385))drawIcon(110,1,5);
    if((battVolts>385)&&(battVolts<=450))drawIcon(110,1,4);
    if(battVolts>450)drawIcon(110,1,41);
    ...
    if(battVolts<=340)
    {
      ...
      display.print(battVolts/100);
      display.print(" V");
      display.setTextSize(2);
      display.setTextColor(BLACK,WHITE);
      display.setCursor(centerX-30, centerY);
 ...
 float thermometer()
 {
   ...
   float celsius = (millivolts / 10)-11;
   return celsius;
 }
...
while(digitalRead(buttonPin3)==LOW){
          ...
          for(int i=0; i<64; i++)
           ...
           display.setTextSize(1);
           display.setTextColor(BLACK ,WHITE);
           display.setCursor(centerX-15,centerY);
